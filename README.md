...
- Career + Tax V2 : rémunérations et événements datés, fiscalité paramétrique versionnée,
  cash net explicable et priorité stricte aux transactions observées.
...
La migration 28 étend cette fondation au domaine comptable, sans la dupliquer : le domaine
...
d'écriture sur `business_financials`. Une période financière déjà renseignée par une AUTRE
source n'est jamais écrasée : seule une correction FEC → FEC l'est, et la preuve d'une origine
...
La migration 26 installe Career + Tax V2 de manière additive. Elle conserve `income_sources`
comme déclaration nette legacy, étend les faits fiscaux, ajoute les rôles, termes, événements,
equity, scénarios, jeux de règles et observations, puis réserve les cinq RPC atomiques au rôle
serveur. Aucun net, résultat fiscal ou barème France n'est persisté ou inventé. Voir
[`docs/CAREER_TAX_ARCHITECTURE.md`](docs/CAREER_TAX_ARCHITECTURE.md).
La migration 27 ajoute les quatre index couvrants révélés par l'advisor PostgreSQL, sans
modifier aucune donnée ni règle métier.
...
- [Architecture Career + Tax V2](docs/CAREER_TAX_ARCHITECTURE.md)
