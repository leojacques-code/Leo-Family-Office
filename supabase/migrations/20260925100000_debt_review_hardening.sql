-- Suites de la relecture indépendante de B16/B17 (25 septembre 2026). Migration ADDITIVE :
-- aucune migration antérieure n'est réécrite.
--
--     PORTE UNIQUE ≠ PORTE PRINCIPALE        CLÉ ABSENTE ≠ DÉCLARATION RETIRÉE
--     UNE PRIME, UN DÉBIT
--
--   * Les tables de dette gardaient les droits INSERT, UPDATE et DELETE accordés à
--     `authenticated` par les premières migrations. Toutes les écritures applicatives passent
--     par le serveur (`service_role`) et les RPC `lfo_*` ; un client muni de la clé publique
--     pouvait pourtant réécrire `terms_status` ou `insurance_mode`, ou supprimer une dette,
--     sans passer par `lfo_save_debt_contract` ni laisser de trace dans
--     `liability_terms_transitions`. Ces droits sont retirés ; la lecture et la RLS restent.
--   * `lfo_save_debt_contract`, reprise de sa DERNIÈRE version (`20260925090000`) :
--       - une dette dont le choix d'assurance est déclaré refuse une charge sans
--         `insurance_mode` (auparavant remis à NULL, polices supprimées) ;
--       - deux périodes de prime d'une même police ne peuvent pas se recouvrir.
