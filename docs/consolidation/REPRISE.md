...
## Sauvegarde des devises Dettes — 22 septembre

Le snapshot final `e10c4c04cb6c1b174734242dbaf51c619117a308` sur `codex/lfo-backup-20260916` contient le source exact du commit local `f96a4eef4d3f8f23a436768517a6913cd71a7b9d`, arbre `904632a134dc876d58aae7d2f1a11f54e0c9ca3e`. Trois archives (historique, contexte, devises Dettes) restaurent ce commit et son historique original dans un dépôt neuf ; comparaison du code et git fsck réussis. Main inchangée.

## Devises des Objectifs — dépendance B14, 23 septembre

Baseline : `f96a4eef4d3f8f23a436768517a6913cd71a7b9d`. Source : document 07 §4 (audit de chaîne et revue indépendante), tranche B14 ouverte sur les consommateurs de devise. Conserver l’évaluation canonique, les versions et les commandes existantes. Réutiliser Currency avec devise explicite ; étendre uniquement le brouillon et la validation de persistance. Aucun calcul ni schéma modifié.

Défaut : la cible, le courant, l’écart et la projection étaient étiquetés EUR ; versionner une cible USD lui réattribuait reportingCurrency même si seul son nom changeait. La cible rend désormais target.currency, les résultats leur observation.currency ; une projection sans observation n’invente aucune devise. Le brouillon d’édition reprend la devise de la cible, et la création capture la devise de lecture au moment de son ouverture. Le libellé Cible et la commande partagent cette valeur. Un changement de contexte pendant la saisie ne réinterprète pas le montant. Aucun sélecteur de devise ni conversion ajouté.

Audit de chaîne : goalDefinitionSchema valide la commande, le repository transmet sa définition à lfo_create_goal_v2 / lfo_save_goal_version_v2, la RPC conserve la devise du payload dans goal_versions et avance la version attendue. Relecture indépendante : le DTO de lecture autorise null mais lfo_validate_goal_definition_v2 refuse SQL NULL pour target.currency. Correction du contre-exemple : le formulaire explique le refus et conserve son brouillon sans envoyer de commande ; l’API refuse également la devise absente avant RPC. Le contrat de lecture nullable reste intact. Cette tranche ne permet pas de déclarer une devise absente depuis le formulaire.

Validation : **17 tests ciblés / 3 fichiers** (7 interface Objectifs, 8 primitive Currency, 2 validation de commande), évaluations financières simulées ; constructeur de définition réel sans calcul. Mêmes tests exécutés indépendamment par le relecteur, verdict final favorable après correction du refus null. Lint, build Next.js avec TypeScript et git diff --check passent. Aucun test de calcul financier relancé. Le test initial qui simulait la réussite d’une sauvegarde null a été remplacé par un refus explicite conforme à SQL.

Recette isolée : nouvelle base locale jetable lfo_goal_currency_20260923, 49 migrations / 107 tables depuis zéro ; profil EUR et objectif USD de fixture créés via RPC. Navigateur → versionner → modifier seulement le nom → enregistrer → recharger → rouvrir : nom repris, cible 1200 et devise USD préservés. SQL confirme versions 1 et 2 avec cible identique, ancien nom conservé en v1 et nouveau nom en v2. Requête HTTP locale de versionnement avec devise null : réponse 400 explicite ; aucune troisième version créée. Le cas null n’a pas été introduit artificiellement dans la base en contournant ses contraintes.

Captures bureau 1280×900 (formulaire rouvert) et mobile 390×844 (fiche) inspectées par l’auteur ; aucune erreur navigateur avant le refus volontaire, aucun débordement horizontal global. Une cible USD devant une lecture EUR reste non calculable sans conversion explicite, comme l’indique déjà le moteur ; aucun résultat financier validé par cette recette. Preuves LFO_devises_objectifs_* dans docs/consolidation/preuves. Base jetable et processus dédiés supprimés ; serveur rétabli sur lfo_auth_gate, espace Objectifs habituel vide confirmé. Auth demeure une fixture, pas une preuve Supabase Auth réelle.

Suite : Decision Lab, Scénarios (en particulier devise du contrat Monte-Carlo) et Immobilier, puis les autres consommateurs et les commandes de création, avant ouverture du choix global. L’affectation de ressources aux objectifs reste en phase 9 ; elle n’est pas ajoutée à cette correction d’unité. B14 et les preuves Auth/Storage A/B restent ouverts selon les limites précédentes. Aucune ressource payante, migration distante ou publication Vercel.

## Sauvegarde des Objectifs — 23 septembre

Après autorisation de publication sur le dépôt public leojacques-code/Leo-Family-Office, le snapshot `b250e9c306e792e9bc23813a63dd88391180dc14` a été publié sur `codex/lfo-backup-20260916`. Source exact : `49835c503df3cc1f7c92800fe45f7c7bf9fb88e2`, arbre `ba9e1453c788323bc247daa5362928f05cb91863`. Récupération depuis GitHub et restauration des quatre archives dans un dépôt neuf : SHA, arbre et git fsck conformes. Le blocage d’autorisation historique est levé. Main inchangée.

## Devises de Decision Lab — dépendance B14, 23 septembre

Baseline : `49835c503df3cc1f7c92800fe45f7c7bf9fb88e2`. Routine document 07 §4 : conserver calculs, cas/version/run et RPC ; réutiliser Currency/OptionalCurrency ; étendre le DTO du résultat à reportingCurrency et ses validations. Aucun sélecteur global, conversion, formule ou migration ajouté.

Chaîne auditée : le contexte global fournit reportingCurrency à evaluateDecisionCase ; celui-ci la copie désormais dans le résultat. Le wrapper global conserve le champ par spread ; save_decision_run_v2 transmet le snapshot complet par structuredClone et lfo_save_decision_run_v2 stocke p_result intact en JSONB. readDecisionResult valide puis retourne l’original, sans supprimer la provenance. Le rapport institutionnel Décision ne rend pas de montant du snapshot : aucun réétiquetage monétaire n’y est ajouté.

Les métriques baseline, terminal et delta affichent la devise du résultat, même après un changement du contexte courant. Les impacts sur les objectifs conservent observation.currency. Les snapshots historiques sans reportingCurrency ou avec null restent lisibles, avec devise explicitement inconnue ; aucune migration EUR implicite. API et lecteur refusent les valeurs malformées, sans transformer leur absence en zéro ou en EUR. Le producteur courant renseigne toujours la devise reçue.

Validation : 14 tests ciblés / 2 fichiers (5 interface, 9 commande/JSON/lecture), résultats financiers saisis et moteurs simulés. Constructeurs de définitions/fingerprint réels, aucun moteur d’évaluation exécuté par ces tests. Relecteur indépendant : mêmes 14 tests réussis, audit SQL/RPC/lecture favorable. Lint, TypeScript, build et diff --check réussis. Les captures ont motivé un complément CSS limité aux deux tableaux : espacement, delta sous le montant, nombres tabulaires et défilement horizontal. Revue statique indépendante favorable ; tests, lint et build repassés après ce complément.

Recette réelle : base locale distincte lfo_decision_currency_20260923, créée depuis zéro avec 49 migrations / 107 tables ; profil USD, compte USD et deux scénarios fictifs préparés via RPC. Bouton « Lancer et enregistrer » → acquittement → SQL et GET /api/state : même run, reportingCurrency USD, aucun snapshotBlocker. Passage temporaire du profil de fixture à CHF, rechargement de la page et nouvelle lecture API : contexte CHF, résultat enregistré toujours USD. POST avec devise malformée « usd » : HTTP 400 ; toujours un seul run SQL. Les moteurs se sont exécutés pour l’affichage réel, sans vérification de leurs résultats numériques.

Limite de rechargement : le snapshot est conservé et relu par le repository/API, mais l’écran existant ne propose pas de sélection d’un cas enregistré et réinitialise sa comparaison courante au rechargement. Ce lot ne livre pas cette navigation d’historique. La capture bureau montre l’acquittement du run enregistré ; la capture mobile est une nouvelle exécution d’affichage en USD, sans nouvel enregistrement. Les deux captures finales sont inspectées par l’auteur, pas par le relecteur. Aucun débordement horizontal global (1280/1280 et 390/390) ; tableau mobile défilable, avertissement de fenêtre large préservé conformément au manifeste Decision Lab. Aucune erreur navigateur avant le refus HTTP volontaire.
