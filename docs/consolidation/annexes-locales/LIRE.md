...
Le code courant et les migrations restent à la racine du dépôt. Son historique Git complet se restaure selon le [guide de restauration sur la branche GitHub de sauvegarde](https://github.com/leojacques-code/Leo-Family-Office/blob/codex/lfo-backup-20260916/docs/consolidation/sauvegarde/RESTAURATION.md) (ce répertoire est ajouté uniquement au snapshot GitHub). Les documents source fournis ne sont pas du travail produit par l’agent ; leurs références et empreintes sont dans `../REPRISE.md`. Les dépendances, caches, bases temporaires et secrets ne sont pas des livrables à publier.
...
Une seconde archive conserve les 52 journaux statiques de validation historiques, avec inventaire distinct. Les sorties financières anciennes ne signifient pas que ces tests ont été réexécutés dans cette tranche : ils restent reportés conformément à la demande du propriétaire. Seuls les journaux runtime actifs PostgreSQL et HTTP sont exclus.
