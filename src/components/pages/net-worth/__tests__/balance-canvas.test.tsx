...
describe("canvas du bilan — la surface encode le montant", () => {
  it("proportionne les hauteurs sur une base commune aux deux colonnes", () => {
    ...
    const heightOf = (node: Element) =>
      Number(
        (node.querySelector(".nw-block-measure") as HTMLElement).style.height.replace("px", ""),
      );
  ...
  it("garde un zéro déclaré mesuré, distinct d’un montant inconnu", () => {
    ...
    expect(liquid!.querySelector(".nw-block-measure")).toHaveStyle({ height: "0px" });
...
it("ne gonfle pas une petite valeur pour loger son libellé", () => {
  const { container } = renderCanvas(
    stateOf({ accounts: [account("a", 1, "BANK"), account("b", 19999, "PEA")] }),
  );
  const measured = container.querySelector(".nw-block-measure") as HTMLElement;
  expect(parseFloat(measured.style.height)).toBeCloseTo(340 / 20000, 6);
  expect(screen.getByText("Liquidités")).toBeVisible();
});

it("garde lisible une famille partiellement valorisée sans mesurer son total inconnu", () => {
  const { container } = renderCanvas(
    stateOf({ accounts: [account("eur", 10000), account("usd", 10000, "BANK", "USD")] }),
  );
  const block = container.querySelector('.nw-block[data-unknown="true"]')!;
  expect(block.querySelector(".nw-block-measure")).toBeNull();
  expect(block).toHaveTextContent("Liquidités");
  expect(block).toHaveTextContent("Non calculable");
  expect(block).toHaveTextContent("Connu");
  expect(screen.getByText(/Leur hauteur n’est pas mesurée/)).toBeVisible();
});
