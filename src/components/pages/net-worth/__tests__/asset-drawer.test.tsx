...
describe("tiroir d’actif — un champ vide n’est jamais un zéro", () => {
  it("reprend le solde historique pour corriger une observation existante", () => {
    ...
    expect(screen.getByLabelText(/Solde observé, en EUR/)).toHaveValue("4200");
  });
  ...
  it("refuse l’enregistrement tant que le solde est absent", async () => {
    ...
    fireEvent.change(screen.getByLabelText(/Date du solde/), { target: { value: "2026-09-08" } });
    await userEvent.click(screen.getByRole("button", { name: /Enregistrer le compte/ }));
  ...
  it("lit une virgule décimale, ce qu’un champ numéro refuse selon la locale", async () => {
    ...
    await userEvent.clear(screen.getByLabelText(/Solde observé/));
...
describe("tiroir d’actif — la date est transmise en création et correction", () => {
  it("exige une date explicite dès la création", () => {
    renderDrawer(null);
    expect(screen.getByLabelText(/Date du solde/)).toHaveValue("");
    expect(screen.getByLabelText(/Date du solde/)).toBeRequired();
  });

  it("exige la date en mise à jour et refuse une observation sans date", async () => {
    const { onSubmit } = renderDrawer(existing);
    fireEvent.change(screen.getByLabelText(/Date du solde/), { target: { value: "" } });
...
describe("tiroir d’actif — forme et devise", () => {
  ...
  it("s’ouvre comme un dialogue, hors du canvas", () => {
    ...
    expect(screen.getByRole("dialog", { name: "Ajouter un compte" })).toBeVisible();
  });
});
