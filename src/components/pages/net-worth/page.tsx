...
function NetWorthPage({
  ...
  busy,
  setExplanation,
}: Pick<SectionProps, "state" | "mutate" | "busy" | "setExplanation">) {
  ...
  useRegisterPrimaryAction(busy ? null : openCreate);

  const assetDrawer = drawerOpen ? (
    <AssetDrawer
      account={selected}
      busy={busy}
  ...
  if (view.isEmpty) {
    return (
      <>
        <EmptyState
          action={
            <button className="button primary" disabled={busy} onClick={openCreate}>
  ...
  return (
    <div className="page-stack">
      ...
      <NetWorthInsights
        closeDate={state.dates?.today}
        busy={busy}
        onCreateClose={() => {
          if (state.dates?.today)
            void mutate({ action: "create_monthly_close", closeDate: state.dates.today });
        }}
        onInspectAllocation={() => {
