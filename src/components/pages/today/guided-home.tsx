...
const INBOX = [
  ...
  { icon: RefreshCw, title: "Vos données ont plusieurs niveaux de fraîcheur", text: "Une vue guidée permet de distinguer ce qui est observé, ancien ou encore hypothétique.", action: "Voir les sources", tone: "cyan" },
...
export default function GuidedHome({ state }: SectionProps) {
  ...
  const kpis = useMemo(() => {
    const debt = state.metrics.totalDebt ?? state.liabilities.reduce((sum, item) => sum + (item.currentBalance ?? 0), 0);
    return {
    ...
    } satisfies Record<KpiKey, { label: string; value: number | null; icon: LucideIcon; hint: string; accent: string }>;
  }, [state]);
    ...
    <div className="guided-home">
      <section className="guided-hero">
        <div>
          ...
          <p>
            LFO ne vous demande pas de penser comme un moteur financier. Il rassemble vos sources,
            relie les faits entre eux et vous guide vers ce qui mérite une décision.
          </p>
        </div>
      ...
      <section className="guided-universe-layout">
        <article className="wealth-universe-panel">
          ...
          <div className="wealth-universe">
            ...
            <div className="wealth-globe">
              <div className="globe-latitude lat-a" />
              <div className="globe-latitude lat-b" />
              <div className="globe-longitude long-a" />
              <div className="globe-longitude long-b" />
              <div className="globe-core">
                <span>YOU</span>
                <strong>{state.metrics.netWorth === null ? "—" : compact.format(state.metrics.netWorth)}</strong>
            ...
            <svg className="universe-links" viewBox="0 0 1000 600" preserveAspectRatio="none" aria-hidden="true">
              <defs>
                <linearGradient id="universeLine" x1="0" x2="1"><stop offset="0" stopColor="#55dcff" stopOpacity=".08"/><stop offset=".5" stopColor="#7d9cff" stopOpacity=".72"/><stop offset="1" stopColor="#a879ff" stopOpacity=".08"/></linearGradient>
              </defs>
              <path d="M500 300 C330 215 260 180 150 130" />
              <path d="M500 300 C650 205 730 150 840 115" />
              <path d="M500 300 C680 300 770 295 910 300" />
              <path d="M500 300 C650 410 700 465 820 500" />
              <path d="M500 300 C360 420 275 455 170 470" />
              <path d="M500 300 C330 300 205 300 95 300" />
            </svg>
            {DOMAIN_NODES.map(({ id, label, icon: Icon, x, y, color }) => (
              <button
                key={id}
                className={`universe-node node-${color} ${activeNode === id ? "active" : ""}`}
                style={{ left: x, top: y }}
          ...
          <div className="universe-context-strip">
            <div className={`context-icon node-${activeDomain.color}`}><activeDomain.icon size={18} /></div>
        ...
        <aside className="financial-inbox-panel">
          ...
          <div className="source-health">
            ...
            {SOURCE_ITEMS.map(({ label, icon: Icon, status, tone }) => (
              <div className="source-health-row" key={label}><Icon size={13}/><span>{label}</span><small className={tone}>{status}</small></div>
            ))}
          </div>
        </aside>
      </section>
      ...
      <section className="data-story-panel">
        <div className="guided-panel-head"><div><span className="eyebrow">How LFO thinks</span><h2>Des sources dispersées vers une décision explicable</h2></div><span className="data-story-caption">survolez les étapes</span></div>
        <div className="data-story-flow">
          <div className="story-node"><span><Database size={16}/></span><strong>Sources</strong><small>Banques, documents, APIs</small></div>
          <div className="story-edge"><i/><i/><i/></div>
          <div className="story-node"><span><FileCheck2 size={16}/></span><strong>Réconciliation</strong><small>Les faits se confirment</small></div>
          <div className="story-edge"><i/><i/><i/></div>
          <div className="story-node"><span><Activity size={16}/></span><strong>Moteurs</strong><small>Cash, dette, fiscalité, valo</small></div>
          <div className="story-edge"><i/><i/><i/></div>
          <div className="story-node featured"><span><WalletCards size={16}/></span><strong>Vérité financière</strong><small>Une vue consolidée</small></div>
          <div className="story-edge"><i/><i/><i/></div>
      ...
      <section className="kpi-story-panel">
        ...
        <div className="kpi-story-tabs">
          {(Object.entries(kpis) as [KpiKey, typeof kpis[KpiKey]][]).map(([key, metric]) => {
          ...
          <div className="kpi-story-chart">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={trend} margin={{ top: 18, right: 12, bottom: 0, left: 0 }}>
                ...
                <XAxis dataKey="period" tickLine={false} axisLine={false} tick={{ fill: "var(--text-muted)", fontSize: 10 }} />
                <YAxis hide domain={["dataMin", "dataMax"]}/>
                <Tooltip formatter={(value) => euro.format(Number(value))} contentStyle={{ borderRadius: 14 }} />
                <Area type="monotone" dataKey="value" stroke="var(--section-accent)" strokeWidth={2.6} fill="url(#guidedArea)" activeDot={{ r: 5 }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          <div className="kpi-explain-card">
            <span className="kpi-explain-label">Pourquoi ce KPI compte</span>
            <h3>{activeMetric.label}</h3>
            <p>{activeKpi === "netWorth" ? "Le patrimoine mesure la solvabilité globale, mais ne dit pas ce qui est immédiatement disponible." : activeKpi === "liquidity" ? "La liquidité répond à une question simple : combien de ressources sont mobilisables sans vendre un actif difficilement cessible ?" : activeKpi === "freedom" ? "La marge mensuelle relie revenus, dépenses, dette et capacité réelle à financer vos projets futurs." : activeKpi === "invested" ? "Le capital investi doit être lu avec son risque, son horizon, sa fiscalité et sa fonction dans vos objectifs." : "La dette n'est pas seulement un montant : calendrier, coût, assurance et capacité de remboursement modifient votre trajectoire."}</p>
            <button>Voir la chaîne de calcul <Network size={13}/></button>
          </div>
        </div>
      </section>
      ...
      <section className="guided-journeys">
        ...
        <div className="journey-grid">
          {JOURNEYS.map(({ icon: Icon, title, text, href, nodes }) => (
            <Link href={href} className="journey-card" key={title}>
              <span className="journey-icon"><Icon size={18}/></span><h3>{title}</h3><p>{text}</p>
              <div className="journey-mini-flow">{nodes.map((node, index) => <span key={node}><i>{node}</i>{index < nodes.length - 1 ? <ChevronRight size={11}/> : null}</span>)}</div>
              <em>Explorer <ArrowRight size={12}/></em>
            </Link>
          ))}
        </div>
      </section>
