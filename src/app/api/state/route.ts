...
function failure(error: unknown, fallback: string) {
  ...
  return NextResponse.json({ error: unauthorized ? "Non authentifié" : fallback }, { status: unauthorized ? 401 : 500 });
}
...
export async function GET() {
  try {
    ...
    return NextResponse.json(await repository.getDashboardState(), { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
