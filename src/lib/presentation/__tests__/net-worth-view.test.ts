...
it("ne signale pas un écart dû à l’ordre des additions décimales", () => {
  const state = stateOf({
    accounts: [account("a", 1794.41), account("b", 333.33, "PEA"), account("c", 0.3)],
  });
  expect(buildNetWorthView(state).assetResidual).toBe(0);
});

it("conserve une véritable différence d’un centime dans le bouclage", async () => {
  const { canonicalBalanceSheetOf } = await import("@/lib/engine/balance-sheet-view");
  const state = stateOf({ accounts: [account("a", 1794.41)] });
  const sheet = canonicalBalanceSheetOf(state);
  state.balanceSheet = {
    ...sheet,
    grossAssets: { ...sheet.grossAssets, knownValue: sheet.grossAssets.knownValue + 0.01 },
  };
  expect(buildNetWorthView(state).assetResidual).toBeCloseTo(-0.01, 10);
});
