 ...
 */
/** Tolérance d’arrondi binaire, bornée sous le centime pour conserver les vrais écarts. */
function groupingResidual(sum: number, total: number): number {
  const residual = sum - total;
  const tolerance = Math.min(
    0.001,
    Number.EPSILON * Math.max(1, Math.abs(sum), Math.abs(total)) * 8,
  );
...
export function buildNetWorthView(state: DashboardState): NetWorthView {
  ...
  const toBlock = (
    ...
    return {
      ...
      // Une famille partiellement connue reste non mesurée ; sa part connue est textuelle.
      weight:
        aggregate.value === null ? null : scaleBasis === null ? 0 : aggregate.value / scaleBasis,
  ...
  return {
    ...
    assetResidual: groupingResidual(knownSum(assets), sheet.grossAssets.knownValue),
    liabilityResidual: groupingResidual(knownSum(liabilities), sheet.totalLiabilities.knownValue),
