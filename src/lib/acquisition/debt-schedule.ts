...
export function summarizeDebtSchedule(rows: Row[]): DebtScheduleSummary {
  ...
  const totalProvidedCost = totals.interest + totals.insurance + totals.fees;
  return {
    ...
    totalProvidedCost: fromCents(totalProvidedCost),
    totalCashOut: fromCents(totals.principal + totalProvidedCost),
  };
}
