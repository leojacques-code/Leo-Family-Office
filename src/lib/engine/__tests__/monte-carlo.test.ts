...
describe("Monte Carlo engine", () => {
  it("is exactly reproducible with a seed", () => {
    const input = { scenario, initialAssets: 15571.49, years: 5, simulations: 500, seed: 19082026 };
    expect(runMonteCarlo(input)).toEqual(runMonteCarlo(input));
  });

  it("returns ordered percentiles for every year", () => {
    const result = runMonteCarlo({ scenario, initialAssets: 15571.49, years: 3, simulations: 1000, seed: 42 });
  ...
  it("applies a deterministic dated shock", () => {
    const stressed = runMonteCarlo({ scenario: { ...scenario, annualVolatility: 0, stressProbability: 0, shockYear: 1, shockMagnitude: -0.35 }, initialAssets: 10000, years: 1, simulations: 100, seed: 1 });
    const base = runMonteCarlo({ scenario: { ...scenario, annualVolatility: 0, stressProbability: 0 }, initialAssets: 10000, years: 1, simulations: 100, seed: 1 });
