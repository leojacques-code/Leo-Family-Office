...
const illustrativeRule: DatedTaxRule = {
  ...
  brackets: [
    ...
    { threshold: 30000, rate: 0.3 },
  ],
  socialContributionsRate: 0,
  source: "Fixture de test, pas une règle française",
...
describe("dated tax rules", () => {
  ...
  it("builds the gross-to-net bridge from explicit inputs", () => {
    const result = employmentCompensation({
      grossFixed: 42000,
      grossVariable: 9000,
      employeeContributionRate: 0.22,
      deductibleAllowanceRate: 0.1,
      taxRule: illustrativeRule,
    });
